...
public class AmazonsqsRiver extends AbstractRiverComponent implements River {
        ...
        public AmazonsqsRiver(RiverName riverName, RiverSettings settings, Client client) {
                ...
                if (settings.settings().containsKey("amazonsqs")) {
                        ...
                        ACCESSKEY = XContentMapValues.nodeStringValue(sqsSettings.get("access_key"), "null");
                        SECRETKEY = XContentMapValues.nodeStringValue(sqsSettings.get("secret_key"), "null");
                ...
                {
                    logger.info(
                        "Setting AWS Credentials to: "
                        .concat("access_key(****")
                        .concat(ACCESSKEY.substring(ACCESSKEY.length()-4))
                        //.concat("access_key(")
                        //.concat(ACCESSKEY)
                        .concat(" length: ")
                        .concat(ACCESSKEY.length()+"")
                        .concat("); ")
                        .concat("secret_key(****")
                        .concat(SECRETKEY.substring(SECRETKEY.length()-4))
                        //.concat("secret_key(")
                        //.concat(SECRETKEY)
                        //.concat(" length: ")
                        .concat(SECRETKEY.length()+"")
                        .concat("); ")
                        .concat("Endpoint: (")
                        .concat(endpoint)
                        .concat("); ")
        ...
        private class Consumer implements Runnable {
                ...
                public void run() {
                        ...
                        int initial_sleeptime = TIMEOUT * 1000;
                        while (!closed) {
                                // pull messages from SQS
                                List<JsonNode> msgs = pullMessages();
                                int sleeptime = initial_sleeptime;
                                
                                if(DEBUG)
                                {
                                    logger.info("SQS scan..");
                                }
